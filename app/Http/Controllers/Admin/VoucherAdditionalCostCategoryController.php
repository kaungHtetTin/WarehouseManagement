...
class VoucherAdditionalCostCategoryController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $rows = VoucherAdditionalCostCategory::query()
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            'name' => [
                ...
                'string',
                'max:255',
                Rule::unique('voucher_additional_cost_categories', 'name')
                    ->where(fn ($q) => $q->where('organization_id', $organizationId)->whereNull('deleted_at')),
            ],
            'status' => ['nullable', Rule::in(['ACTIVE', 'INACTIVE'])],
            'sort_order' => ['nullable', 'integer', 'min:0', 'max:1000000'],
        ]);

        $row = VoucherAdditionalCostCategory::query()->create([
        ...
        AuditLogger::record($actor, 'voucher_additional_cost_category.create', $row, [
            'name' => $row->name,
        ]);
    ...
    public function update(Request $request, string $category): RedirectResponse
    {
        ...
        AuditLogger::record($actor, 'voucher_additional_cost_category.update', $categoryModel, [
    ...
    public function destroy(Request $request, string $category): RedirectResponse
    {
        ...
        AuditLogger::record($actor, 'voucher_additional_cost_category.delete', null, $snapshot);

        return Redirect::route('admin.voucher-additional-cost-categories.index')
            ->with('success', 'Category deleted successfully.');
    }

    private function resolveTenantCategory(User $user, string $id): VoucherAdditionalCostCategory
    {
        abort_if($user->organization_id === null, 404);

        return VoucherAdditionalCostCategory::query()
