...
| Phase 1 | **Partial** | Tenant + RBAC + IAM UI/API done; `user_warehouse_access` scope, audit log persistence, and PHPUnit `APP_URL` baseline added—expand automated tests beyond Phase 1 samples.                                                                                                |
| Phase 2 | **Partial** | Master data: warehouses, categories, products, merchants, vehicles + stock by warehouse (`warehouse_stocks`, `stock_movements`, manual ADJUSTMENT). Stock list hides zero-qty rows. Remaining: automated movement types beyond adjustment, richer stock UX.               |
| Phase 3 | **Partial** | Vouchers + trips MVP implemented (draft CRUD + wizard confirm + trips load-from-confirmed). Voucher detail exists and supports payment recording + compact payment list UI. Remaining: delivery confirmations, voucher lifecycle statuses, stock linkage, print/manifest. |
| Phase 4 | **Open**    | Public portfolio page per organization not started.                                                                                                                                                                                                                       |
| Phase 5 | **Open**    | Subscription and billing not started.                                                                                                                                                                                                                                     |
| Phase 6 | **Open**    | Reporting, hardening, and UAT not started.                                                                                                                                                                                                                                |
| Phase 7 | **Open**    | Production launch and stabilization not started.                                                                                                                                                                                                                          |
...
- [x] Warehouse create auto-assign access — creator + org super admins get `MANAGE` on new warehouse.
...
**Status: Partial** — Master data CRUD (warehouses, categories, products, merchants, vehicles) and stock listing + signed quantity adjustments (`/admin/inventory/stocks`) are in place. Stock listing hides rows where both on-hand and reserved are 0. Voucher-driven movements remain Phase 3.
...
- Stock listing UX — **implemented** (filters out zero-qty rows).
...
- Voucher/line payment status tracking (UNPAID/PARTIAL/PAID) + payment recording/list UI — **implemented**.
